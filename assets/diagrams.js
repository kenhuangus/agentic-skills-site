...
(function () {
  ...
  function drawTop10List(host) {
    ...
    const gapX = 32;
    const colW = Math.floor((innerW - gapX) / 2);
    const rowH = 72;
    const startX = [innerX, innerX + colW + gapX];
    const startY = 88;
    const dividerX = innerX + colW + gapX / 2;
    ...
    risks.forEach((risk, idx) => {
      ...
      chip(s, x + 16, y + 10, 58, 38, risk.code, risk.col, { size: 11.5, weight: 700, fill: hex(risk.col, 0.18) });
      txt(s, x + 84, y + 35, risk.title, { size: 14.5, weight: 700, col: COL.ink });
    });

    line(s, dividerX, startY, dividerX, startY + rowH * 5 - 14, { stroke: COL.line, strokeWidth: 1.2 });
  }
  ...
  function drawRiskMap(host) {
    ...
    // Top Header Badge
    chip(s, 240, 10, 400, 42, zh ? "OWASP Agentic Skills Top 10 · 核心风险全景架构" : "OWASP Agentic Skills Top 10 · Executive Risk Taxonomy", COL.accent, { size: 13.5, fill: hex(COL.accent, 0.15) });

    // Central Divider Axes (strictly in the empty gutters between quadrants)
    line(s, 30, 275, 850, 275, { stroke: COL.line, strokeWidth: 1.2 });
    line(s, 440, 65, 440, 485, { stroke: COL.line, strokeWidth: 1.2 });

    // Q1: Sourcing & Registry Trust (Top-Left: x: 30..425, y: 65..265)
    rect(s, 30, 65, 395, 200, { fill: hex(COL.blue, 0.05), stroke: COL.blue, strokeWidth: 2 });
    txt(s, 45, 90, zh ? "1. 技能来源与注册表信任 (Sourcing & Trust)" : "1. Skill Sourcing & Registry Trust", { size: 13.5, weight: 700, col: COL.blue });
    chip(s, 45, 105, 175, 62, zh ? "AST01 恶意技能\n(AMOS木马 / 记忆后门)" : "AST01 Malicious Skills\n(AMOS Stealer / SOUL.md)", COL.blue, { size: 11 });
    chip(s, 235, 105, 175, 62, zh ? "AST02 供应链妥协\n(依赖混淆 / 钩子RCE)" : "AST02 Supply Chain\n(Dependency Confusion / RCE)", COL.teal, { size: 11 });
    chip(s, 140, 182, 175, 62, zh ? "AST04 不安全元数据\n(品牌仿冒 / YAML反序列化)" : "AST04 Insecure Metadata\n(Spoofing / YAML RCE)", COL.grape, { size: 11 });

    // Q2: Execution Boundaries & Privilege (Top-Right: x: 455..850, y: 65..265)
    rect(s, 455, 65, 395, 200, { fill: hex(COL.orange, 0.05), stroke: COL.orange, strokeWidth: 2 });
    txt(s, 470, 90, zh ? "2. 执行边界与特权隔离 (Execution Boundaries)" : "2. Execution Boundaries & Privilege", { size: 13.5, weight: 700, col: COL.orange });
    chip(s, 470, 105, 175, 62, zh ? "AST03 特权过大技能\n(LPCI逻辑注入 / 删库)" : "AST03 Over-Privileged\n(LPCI Injection / DB Wipe)", COL.orange, { size: 11 });
    chip(s, 660, 105, 175, 62, zh ? "AST05 不受信任外部指令\n(作者跑路 / 中继放大)" : "AST05 External Instructions\n(Rug-Pull / Relay Drift)", COL.red, { size: 11 });
    chip(s, 565, 182, 175, 62, zh ? "AST06 弱隔离机制\n(宿主逃逸 / Localhost跨域)" : "AST06 Weak Isolation\n(Host Escape / Loopback)", COL.cyan, { size: 11 });

    // Q3: Lifecycle Governance & Defense (Bottom-Left: x: 30..425, y: 285..485)
    rect(s, 30, 285, 395, 200, { fill: hex(COL.yellow, 0.05), stroke: COL.yellow, strokeWidth: 2 });
    txt(s, 45, 310, zh ? "3. 全生命周期治理与防御 (Lifecycle Governance)" : "3. Lifecycle Governance & Defense", { size: 13.5, weight: 700, col: COL.yellow });
    chip(s, 45, 325, 175, 62, zh ? "AST07 更新漂移\n(静默带毒升级 / 热重载)" : "AST07 Update Drift\n(Silent Auto-Update / Drift)", COL.yellow, { size: 11 });
    chip(s, 235, 325, 175, 62, zh ? "AST08 扫描能力不足\n(自然语言免杀 / 截断失明)" : "AST08 Poor Scanning\n(NL Evasion / Truncation)", COL.pink, { size: 11 });
    chip(s, 140, 402, 175, 62, zh ? "AST09 治理缺失\n(影子AI / 缺乏双向审计)" : "AST09 No Governance\n(Shadow AI / Audit Gaps)", COL.indigo, { size: 11 });

    // Q4: Cross-Platform Interoperability (Bottom-Right: x: 455..850, y: 285..485)
    rect(s, 455, 285, 395, 200, { fill: hex(COL.green, 0.05), stroke: COL.green, strokeWidth: 2 });
    txt(s, 470, 310, zh ? "4. 跨平台重用与标准 (Cross-Platform)" : "4. Cross-Platform Interoperability", { size: 13.5, weight: 700, col: COL.green });
    chip(s, 470, 325, 365, 62, zh ? "AST10 跨平台重用\n(清单剥离 / 隐式提权 / 跨市场套利)" : "AST10 Cross-Platform Reuse\n(Manifest Stripping / Privilege Loss / Arbitrage)", COL.green, { size: 11.5 });
    chip(s, 470, 402, 365, 64, zh ? "★ 核心解法：通用技能格式 (USF v1.0)\n密码学绑定 + 默认保护 SOUL.md / MEMORY.md" : "★ Core Solution: Universal Skill Format (USF v1.0)\nCryptographic Hashes + Default-Deny Identity Protection", COL.accent, { size: 11, fill: hex(COL.accent, 0.18) });
  }
